...
export default function HomePage() {
  ...
  const handleBillSubmit = async (data: BillSubmitData) => {
    if (!currentUser) {
      alert(t('bill.userNotSync') || 'User not synchronized. Please wait a few seconds or refresh the page.');
      return;
    }
    try {
      setLoading(true);
  ...
  return (
    <div className="min-h-screen notebook-page">
          ...
          <>
            {currentUser && (
              ...
              >
                <h2 className="text-xl font-handwritten text-ink">
                  {t('home.welcome', { name: currentUser.username || t('home.welcomeFriend') })}
                </h2>
              </motion.div>
            )}
            ...
            <div className="flex gap-6 mb-8 mt-4 border-b-2 border-accent/20 pb-1">
              <button
                ...
                className={`px-6 py-3 font-handwritten text-xl transition-all relative ${activeTab === 'active' ? 'text-accent font-bold' : 'text-ink/60 hover:text-ink hover:bg-paper-highlight/50 rounded-t-lg'}`}
              >
                {t('home.activeBills')}
                {activeTab === 'active' && <motion.div layoutId="activeTab" className="absolute bottom-[-3px] left-0 right-0 h-1 bg-accent rounded-full" />}
              </button>
              <button
                onClick={() => setActiveTab('closed')}
                className={`px-6 py-3 font-handwritten text-xl transition-all relative ${activeTab === 'closed' ? 'text-accent font-bold' : 'text-ink/60 hover:text-ink hover:bg-paper-highlight/50 rounded-t-lg'}`}
              >
                {t('home.history')}
                {activeTab === 'closed' && <motion.div layoutId="activeTab" className="absolute bottom-[-3px] left-0 right-0 h-1 bg-accent rounded-full" />}
              </button>
            </div>
            ...
            <div className="min-h-[500px]">
              {activeTab === 'active' && (
                <div>
                  {activeBills.length === 0 ? (
                    <motion.div className="flex flex-col items-center justify-center py-24 px-4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                      <div className="bg-paper-highlight p-12 rounded-2xl border-2 border-dashed border-accent/30 shadow-sm max-w-md w-full flex flex-col items-center text-center transform rotate-1">
                        <div className="mt-2 space-y-2">
                          <p className="text-xl font-handwritten text-ink font-bold">{t('home.noActiveBills')}</p>
                          <p className="text-ink/60 font-handwritten text-lg">{t('home.createFirstBill')}</p>
                        </div>
                      </div>
                    </motion.div>
                  ) : (
                    <div className="grid gap-6">
                      {activeBills.map(bill => (
                         <BillCard key={bill.id} bill={bill} role={bill.owner_id === currentUser?.id ? 'creator' : 'participant'} />
                      ))}
                    </div>
                  )}
                </div>
                  ...
                  {closedBills.length === 0 ? (
                    <motion.div className="flex flex-col items-center justify-center py-24 px-4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                      <div className="bg-paper-highlight p-12 rounded-2xl border-2 border-dashed border-accent/30 shadow-sm max-w-md w-full flex flex-col items-center text-center transform -rotate-1">
                        ...
                        <p className="text-2xl font-handwritten text-ink font-bold">{t('home.noClosedBills')}</p>
                        <p className="mt-2 text-ink/60 font-handwritten text-lg">{t('home.completedBillsNote')}</p>
                      </div>
                    </motion.div>
                  ) : (
                    <div className="grid gap-6">
              ...
              <div ref={loadMoreRef} className="h-10 flex items-center justify-center mt-8">
                {loadingMore && <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-accent"></div>}
              </div>
            </div>
          </>
        )}
      ...
      <FloatingCreateButton onClick={handleCreateBill} show={true} />
      <CreateBillModal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} onSubmit={handleBillSubmit} />
    </div>
  );
}
